...
## Project Status
🚦 **Project Status:** Splitter complete · Identifier in build
...
* **Pluggable Models:** The identifier talks to a model by running a command
  line tool you already have, calling an HTTP API, or a local runtime — chosen
  from a dropdown with a **Test** button that runs it and shows the reply. No
  vendor integration to wait on, and no credential ever stored in a file.
* **A Setup Tab:** Everything done once in one place — what is installed, how to
  install it on this platform, and how to connect a tool. The workflow tabs
  carry a one-line status instead, so a panel nobody reads never sits above the
  work.
...
* Backend adapters — CLI, HTTP API, local runtime — behind one interface,
  configured from the UI and testable with one click.
...
* `src/identifier/`: Observation, interpretation, matching, renaming, export.
* `src/backends/`: The model adapters — CLI, HTTP API, local runtime.
* `src/identifier/templates.py`: Film vocabulary, shipped with the app and never shown.
* `src/ui/`: The app, one routes module per tab, and the browser front end.
* `production/`: A show's own markdown — characters, props, environments. Contents gitignored.
